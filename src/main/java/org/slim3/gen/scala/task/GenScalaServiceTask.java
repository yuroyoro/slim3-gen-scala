...
import org.slim3.gen.scala.generator.ScalaServiceGenerator;
import org.slim3.gen.scala.generator.ScalaServiceTestCaseGenerator;
import org.slim3.gen.task.ClassNameBuilder;
import org.slim3.gen.task.WebConfig;

/**
 * Represents a task to generate a service scala file.
 * 
 * @author yuroyoro
 * @since 3.0
 */
public class GenScalaServiceTask extends AbstractGenScalaFileTask {
    ...
    public void doExecute() throws Exception {
        ...
        ScalaFile scalaFile = createScalaFile(serviceDesc);
        Generator generator = createServiceGenerator(serviceDesc);
        generateScalaFile(generator, scalaFile);

        ScalaFile testCaseScalaFile = createTestCaseScalaFile(serviceDesc);
        Generator testCaseGenerator =
            createServiceTestCaseGenerator(serviceDesc);
        generateScalaFile(testCaseGenerator, testCaseScalaFile);
    }
    ...
    protected Generator createServiceGenerator(ServiceDesc serviceDesc) {
        return new ScalaServiceGenerator(serviceDesc);
    }
    ...
    protected Generator createServiceTestCaseGenerator(ServiceDesc serviceDesc) {
        return new ScalaServiceTestCaseGenerator(serviceDesc);
    }
}
